export default{
    users: [
        {
            'icon':'img-icon.png',
            'id': 0,
            'fname': 'Angela L',
            'lname': 'Alvarez',
            'title':'Alvarez, Angela L',
            'last_seen':' 4/17/2018',
            'action':'profile-details',
            'image':0,
            'day':'Today',
            'assigned':0,
            'dob': '03-28-1984',
            'type': 'HF',
            'height': "5'2\"",
            'weight': "110 lb",
            'sex': 'Female',
            'phone': '(444)-555-3333',
            'profile_image': 'profile-pictures.jpg',
            'aliases': 'Mira',
            'hf': "5'2''-110 lb",
            'mrn': '13245678',
            'ssn': '555-987-6543',
            'images': [
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
            ],
             'alert':[
                {
                    'title':'Suicide threat',
                    'high':1,
                    'type':'Safety',
                    'date':'04-8-2018'
                },
                {
                    'title':'SF HOT request: Checking on leg Wound',
                    'high':0,
                    'type':'Outreach',
                    'date':'04-8-2018'


                },
                {
                    'title':'SF HOT request: Checking on leg Wound',
                    'high':0,
                    'type':'Follow-up',
                    'date':'04-8-2018'


                },
                {
                    'title':'SF HOT request: Checking on leg Wound',
                    'high':0,
                    'type':'Immunization',
                    'date':'04-8-2018'


                },
                {
                    'title':'SF HOT request: Checking on leg Wound',
                    'high':0,
                    'type':'Clinic',
                    'date':'04-8-2018'


                },
                {
                    'title':'SF HOT request: Checking on leg Wound',
                    'high':0,
                    'type':'Safety',
                    'date':'04-8-2018'


                },
                {
                    'title':'SF HOT request: Checking on leg Wound',
                    'high':0,
                    'type':'Outreach',
                    'date':'04-8-2018'


                },
                {
                    'title':'SF HOT request: Checking on leg Wound',
                    'high':0,
                    'type':'Follow-up',
                    'date':'04-8-2018'


                },
            ],
            'allergies': ['Eggs, latex'],
            'medications': [
                {'name':'XYZ', 'prescribed':'4/10/2018'},
                {'name':'XYZ', 'prescribed':'4/10/2018'}
            ],
            'diagnosis': ["HIV (B20)", "Opiate use disorder (F11.24)"]
        },
        {
            'icon':'profile-pictures.jpg',
            'id': 1,
            'fname': 'Mirabel S',
            'lname': 'Gonzalez',
            'title':'Gonzalez, Mirabel S',
            'last_seen':' 4/10/2018',                
            'action':'profile-details',
            'image':1,
            'day':'Today',
            'assigned':1,
            'dob': '04-28-1984',
            'type': 'WM',
            'height': "5'8\"",
            'weight': "170 lb",
            'sex': 'Female',
            'phone': '(444)-555-4444',
            'profile_image': 'profile-pictures.jpg',
            'aliases': 'Mira','hf': "5'2''-110 lb",
            'mrn': '13245678',
            'ssn': '555-987-6543',
            'images': [
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
            ],
           'alert':[
                {
                    'title':'Need Hep a Vaccination',
                    'high':0,
                },
              
            ],
            'allergies': ['Eggs, latex'],
            'medications': [
                {'name':'XYZ', 'prescribed':'4/10/2018'},
                {'name':'XYZ', 'prescribed':'4/10/2018'}
            ],
            'diagnosis': ["HIV (B20)", "Opiate use disorder (F11.24)"]                
        },
        {
            'icon':'img-icon.png',
            'id': 2,
            'fname': 'Carmen A',
            'lname': 'Nardi',
            'title':' Nardi, Carmen A',
            'last_seen':' 1/12/2018',                
            'action':'profile-details',
            'image':0,
            'day':'Today',
            'assigned':0,
            'dob': '05-28-1984',
            'type': 'WM',
            'height': "5'8\"",
            'weight': "180 lb",
            'sex': 'Male',
            'phone': '(444)-555-5555',
            'profile_image': 'profile-pictures.jpg',
            'aliases': 'Mira',
            'hf': "5'2''-110 lb",
            'mrn': '13245678',
            'ssn': '555-987-6543',
            'images': [
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
            ],
             'alert':[
                {
                    'title':'Need Flomax Refill',
                    'high':0,
                    'type':''

                },
                {
                    'title':'Needs Social Worker Conection',
                    'high':0,
                    'type':''

                },
                {
                    'title':'Needs Social Worker Conection',
                    'high':1,
                    'type':''

                },
                {
                    'title':'Needs Social Worker Conection',
                    'high':1,
                    'type':''

                },
                {
                    'title':'Needs Social Worker Conection',
                    'high':1,
                    'type':''

                },
                {
                    'title':'Needs Social Worker Conection',
                    'high':1,
                    'type':''

                },
                
            ],
            'allergies': ['Eggs, latex'],
            'medications': [
                {'name':'XYZ', 'prescribed':'4/10/2018'},
                {'name':'XYZ', 'prescribed':'4/10/2018'}
            ],
            'diagnosis': ["HIV (B20)", "Opiate use disorder (F11.24)"]                                             
        },
        {
            'icon':'img-icon.png',
            'id': 3,
            'fname': 'Ray S',
            'lname': 'Putnam',
            'title':' Putnam, Ray S',
            'last_seen':' 6/22/2018',                
            'action':'profile-details',
            'image':0,
            'day':'Today',
            'assigned':0,
            'dob': '06/28-1984',
            'type': 'WM',
            'height': "5'8\"",
            'weight': "180 lb",
            'sex': 'Male',
            'phone': '(444)-555-6666',
            'profile_image': 'profile-pictures.jpg',
            'aliases': 'Mira',
            'hf': "5'2''-110 lb",
            'mrn': '13245678',
            'ssn': '555-987-6543',
            'images': [
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
            ],
            'allergies': ['Eggs, latex'],
            'alert':[
                {
                    'title':'Suicide threat',
                    'high':1,
                    'type':''

                },
                {
                    'title':'SF HOT request: Checking on leg Wound',
                    'high':0,
                    'type':''

                },
                {
                    'title':'SF HOT request: Checking on leg Wound',
                    'high':0,
                    'type':''

                },
                {
                    'title':'SF HOT request: Checking on leg Wound',
                    'high':0,
                    'type':''

                },
                {
                    'title':'SF HOT request: Checking on leg Wound',
                    'high':0,
                    'type':''

                },
                {
                    'title':'SF HOT request: Checking on leg Wound',
                    'high':0,
                    'type':''

                },
                {
                    'title':'SF HOT request: Checking on leg Wound',
                    'high':0,
                    'type':''

                },
                {
                    'title':'SF HOT request: Checking on leg Wound',
                    'high':0,
                    'type':''

                },
            ],
            'medications': [
                {'name':'XYZ', 'prescribed':'4/10/2018'},
                {'name':'XYZ', 'prescribed':'4/10/2018'}
            ],
            'diagnosis': ["HIV (B20)", "Opiate use disorder (F11.24)"]                                                            
        },
        {
            'icon':'img-icon.png',
            'id': 4,
            'fname': 'Candance B',
            'lname': 'White',
            'title':' White, Candance B',
            'last_seen':' 4/17/2018',                
            'action':'profile-details',
            'image':0,
            'day':'2d',
            'assigned':0,
            'dob': '07/28-1984',
            'type': 'WM',
            'height': "5'8\"",
            'weight': "170 lb",
            'sex': 'Female',
            'phone': '(444)-555-7777',
            'profile_image': 'profile-pictures.jpg',
            'aliases': 'Mira',
            'hf': "5'2''-110 lb",
            'mrn': '13245678',
            'ssn': '555-987-6543',
            'images': [
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
            ],
            'alert':[
                {
                    'title':'Need Flomax Refill',
                    'high':0,
                },
                {
                    'title':'Needs Social Worker Conection',
                    'high':0,
                },
                {
                    'title':'Needs Social Worker Conection',
                    'high':1,
                },
                {
                    'title':'Needs Social Worker Conection',
                    'high':1,
                },
                {
                    'title':'Needs Social Worker Conection',
                    'high':1,
                },
                {
                    'title':'Needs Social Worker Conection',
                    'high':0,
                },
                
            ],
            'allergies': ['Eggs, latex'],
            'medications': [
                {'name':'XYZ', 'prescribed':'4/10/2018'},
                {'name':'XYZ', 'prescribed':'4/10/2018'}
            ],
            'diagnosis': ["HIV (B20)", "Opiate use disorder (F11.24)"]
            
        }
    ],
    alerts: [
        {
            'id': 0,
            'title':'Suicide threat',
            'action':'alert-details',
            'date':'4/17/2018',
            'type':"Safety"
        },
        {
            'id': 1,
            'title':'SF Hot request: Check on log wound',
            'action':'alert-details',
            'date':'4/17/2018',
            'type':"outreach"
        },
        {
            'id': 2,
            'title':'Missed follow-up for wound care',
            'action':'alert-details',
            'date':'4/17/2018',
            'type':"follow-up "
        },
         {
            'id': 1,
            'title':'SF Hot request: Check on log wound',
            'action':'alert-details',
            'date':'4/17/2018',
            'type':"outreach"
        },
         {
            'id': 2,
            'title':'Missed follow-up for wound care',
            'action':'alert-details',
            'date':'4/17/2018',
            'type':"follow-up "
        }
    ],
    followUps: [
        {
            'id': 0,
            'lastUpdated': '03-10-2016',
            'lastUpdatedBy': "Alvarez, Angela L",
            'title': 'Eu duis ultrices, dignissim at fuga',
            'data': 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum',
            'series': 1
        },
        {
            'id': 1,
            'lastUpdated': '03-10-2016',
            'lastUpdatedBy': "Alvarez, Angela L",
            'title': 'Eu duis ultrices, dignissim at fuga',
            'data': 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum',
            'series': 2
        },
        {
            'id': 2,
            'lastUpdated': '03-10-2016',
            'lastUpdatedBy': "Alvarez, Angela L",
            'title': 'Eu duis ultrices, dignissim at fuga',
            'data': 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum',
            'series': 3
        },
        {
            'id': 3,
            'lastUpdated': '03-10-2016',
            'lastUpdatedBy': "Alvarez, Angela L",
            'title': 'Eu duis ultrices, dignissim at fuga',
            'data': 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum',
            'series': 4
        },
    ],
    followUps_details:{'last_updated':'03-10-2016','updated_by':'Alvarez, Angela L'},
    data:[
        {
            'icon':'zmdi-pin',
            'title':'8th St and Mission St,San Fransisco',
            'list_title':'8th St and Mission St.',
            'map_title':'8th St and Mission St,San Fransisco',
            'country':'San Fransisco',
            'date':'14/10/2018',
            'sub_titles':'12 encounters',
            'radius':12,
            'action':'',
            'image':0,
            'day':'Today',
            'assigned':0
        },
        {
            'icon':'zmdi-pin',
            'title':'9th St and Market St,San Fransisco',
            'list_title':'8th St and Mission St',
            'map_title':'9th St',
            'country':'San Fransisco',
            'sub_titles':'7 encounters',
            'date':'14/10/2018',
            'radius':7,                
            'action':'',
            'image':1,
            'day':'Today',
            'assigned':1                
        },
        {
            'icon':'zmdi-pin',
            'title':'Open Access Clinic(50 Ivy),San Fransisco',
            'list_title':'8th St and Mission St',
            'map_title':'OAC',
            'country':'San Fransisco',
            'sub_titles':'4 encounters',
            'date':'14/10/2018',
            'radius':4,                
            'action':'',
            'image':0,
            'day':'Today',
            'assigned':0                                            
        },
    ],
    locationData: [
        {
            id:0,
            title : "Open access -50 lvy",
        },
        {
            id: 1,
            title : "HOSPITAL / SNF",
        },
        {
            id: 2,
            title : "6th St HR Center",
        },
        {
            id: 3,
            title : "Outreach",
        },

    ],
    diagnosesData: [
        {
            id: 0,
            title: "Abdominal Pain R10.84"
        },
        {
            id: 1,
            title: "Absecess (unspecified) L02.91"
        },
        {
            id: 3,
            title: "ADHD F90.0"
        },
    ],
    ol:[
        {
            'icon':'zmdi-accounts',
            'title':'Outreach List',
            'action':'ol'

        },
        {
            'icon':'zmdi-search',
            'title':'Person Search',
            'action':'search'
        },
        {
            'icon':'zmdi-assignment-o',
            'title':'New Encounter Form',
            'action':'nef'
        },
        {
            'icon':'zmdi-receipt',
            'title':'New Quick Note',
            'action':'nqn'                
        },
        {
            'icon':'zmdi-sign-in',
            'title':'Sign Out',
            'action':'login'
            
        }
    ]
};