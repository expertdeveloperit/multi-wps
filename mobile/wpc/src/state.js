export default{
    users: [
        {
            'icon':'img-icon.png',
            'id': 0,
            'fname': 'Angela L',
            'lname': 'Alvarez',
            'title':'Alvarez, Angela L',
            'last_seen':' 4/17/2018',
            'action':'profile-details',
            'image':1,
            'day':'Today',
            'assigned':0,
            'dob': '03/28-1984',
            'sex': 'Female',
            'phone': '444-555-3333',
            'profile_image': 'profile-pictures.jpg',
            'images': [
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
            ],
            'allergies': ['Eggs, latex'],
            'medications': [
                {'name':'XYZ', 'prescribed':'4/10/2018'},
                {'name':'XYZ', 'prescribed':'4/10/2018'}
            ],
            'diagnosis': ["HIV (B20)", "Opiate use disorder (F11.24)"]
        },
        {
            'icon':'profile-pictures.jpg',
            'id': 1,
            'fname': 'Mirabel S',
            'lname': 'Gonzalez',
            'title':'Gonzalez, Mirabel S',
            'last_seen':' 4/10/2018',                
            'action':'profile-details',
            'image':1,
            'day':'Today',
            'assigned':1,
            'dob': '04/28-1984',
            'sex': 'Female',
            'phone': '444-555-4444',
            'profile_image': 'profile-pictures.jpg',
            'images': [
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
            ],
            'allergies': ['Eggs, latex'],
            'medications': [
                {'name':'XYZ', 'prescribed':'4/10/2018'},
                {'name':'XYZ', 'prescribed':'4/10/2018'}
            ],
            'diagnosis': ["HIV (B20)", "Opiate use disorder (F11.24)"]                
        },
        {
            'icon':'img-icon.png',
            'id': 2,
            'fname': 'Carmen A',
            'lname': 'Nardi',
            'title':' Nardi, Carmen A',
            'last_seen':' 1/12/2018',                
            'action':'profile-details',
            'image':1,
            'day':'Today',
            'assigned':0,
            'dob': '05/28-1984',
            'sex': 'Male',
            'phone': '444-555-5555',
            'profile_image': 'profile-pictures.jpg',
            'images': [
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
            ],
            'allergies': ['Eggs, latex'],
            'medications': [
                {'name':'XYZ', 'prescribed':'4/10/2018'},
                {'name':'XYZ', 'prescribed':'4/10/2018'}
            ],
            'diagnosis': ["HIV (B20)", "Opiate use disorder (F11.24)"]                                             
        },
        {
            'icon':'img-icon.png',
            'id': 3,
            'fname': 'Ray S',
            'lname': 'Putnam',
            'title':' Putnam, Ray S',
            'last_seen':' 6/22/2018',                
            'action':'profile-details',
            'image':1,
            'day':'Today',
            'assigned':0,
            'dob': '06/28-1984',
            'sex': 'Male',
            'phone': '444-555-6666',
            'profile_image': 'profile-pictures.jpg',
            'images': [
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
            ],
            'allergies': ['Eggs, latex'],
            'medications': [
                {'name':'XYZ', 'prescribed':'4/10/2018'},
                {'name':'XYZ', 'prescribed':'4/10/2018'}
            ],
            'diagnosis': ["HIV (B20)", "Opiate use disorder (F11.24)"]                                                            
        },
        {
            'icon':'img-icon.png',
            'id': 4,
            'fname': 'Candance B',
            'lname': 'White',
            'title':' White, Candance B',
            'last_seen':' 4/17/2018',                
            'action':'profile-details',
            'image':1,
            'day':'2d',
            'assigned':0,
            'dob': '07/28-1984',
            'sex': 'Female',
            'phone': '444-555-7777',
            'profile_image': 'profile-pictures.jpg',
            'images': [
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
            ],
            'allergies': ['Eggs, latex'],
            'medications': [
                {'name':'XYZ', 'prescribed':'4/10/2018'},
                {'name':'XYZ', 'prescribed':'4/10/2018'}
            ],
            'diagnosis': ["HIV (B20)", "Opiate use disorder (F11.24)"]
            
        }
    ],
    alerts: [
        {
            'id': 0,
            'title':'Suicide threat',
            'action':'alert-details',
            'date':'4/17/2018',
            'type':"Safety"
        },
        {
            'id': 1,
            'title':'SF Hot request: Check on log wound',
            'action':'alert-details',
            'date':'4/17/2018',
            'type':"outreach"
        },
        {
            'id': 2,
            'title':'Missed follow-up for wound care',
            'action':'alert-details',
            'date':'4/17/2018',
            'type':"follow-up "
        }
    ],
    followUps: [
        {
            'id': 0,
            'lastUpdated': '03-10-2016',
            'lastUpdatedBy': "Alvarez, Angela L",
            'title': 'Eu duis ultrices, dignissim at fuga',
            'data': 'Suspendisse nam, posuers nisl vitae tellus, elit ut libero, nobis nunc sed sit...',
            'series': 1
        },
        {
            'id': 1,
            'lastUpdated': '03-10-2016',
            'lastUpdatedBy': "Alvarez, Angela L",
            'title': 'Eu duis ultrices, dignissim at fuga',
            'data': 'Suspendisse nam, posuers nisl vitae tellus, elit ut libero, nobis nunc sed sit...',
            'series': 2
        },
        {
            'id': 2,
            'lastUpdated': '03-10-2016',
            'lastUpdatedBy': "Alvarez, Angela L",
            'title': 'Eu duis ultrices, dignissim at fuga',
            'data': 'Suspendisse nam, posuers nisl vitae tellus, elit ut libero, nobis nunc sed sit...',
            'series': 3
        },
        {
            'id': 3,
            'lastUpdated': '03-10-2016',
            'lastUpdatedBy': "Alvarez, Angela L",
            'title': 'Eu duis ultrices, dignissim at fuga',
            'data': 'Suspendisse nam, posuers nisl vitae tellus, elit ut libero, nobis nunc sed sit...',
            'series': 4
        },
    ],
    data:[
        {
            'icon':'zmdi-pin',
            'title':'8th St and Mission St,San Fransisco',
            'sub_titles':'12 encounters',
            'action':'',
            'image':1,
            'day':'Today',
            'assigned':0
        },
        {
            'icon':'zmdi-pin',
            'title':'9th St and Market St,San Francisco',
            'sub_titles':'7     ',                
            'action':'',
            'image':1,
            'day':'Today',
            'assigned':1                
        },
        {
            'icon':'zmdi-pin',
            'title':'Open access Clinin(50 Ivy),San Francisco',
            'sub_titles':'4 encounters',                
            'action':'',
            'image':1,
            'day':'Today',
            'assigned':0                                            
        },
        
    ],
    history: [
        {
            'icon':'img-icon.png',
            'id': 0,
            'fname': 'Angela L',
            'lname': 'Alvarez',
            'title':'Alvarez, Angela L',
            'last_seen':' 4/17/2018',
            'action':'profile-details',
            'image':1,
            'day':'Today',
            'assigned':0,
            'dob': '03/28-1984',
            'sex': 'Female',
            'phone': '444-555-3333',
            'profile_image': 'profile-pictures.jpg',
            'images': [
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
            ],
            'allergies': ['Eggs, latex'],
            'medications': [
                {'name':'XYZ', 'prescribed':'4/10/2018'},
                {'name':'XYZ', 'prescribed':'4/10/2018'}
            ],
            'diagnosis': ["HIV (B20)", "Opiate use disorder (F11.24)"]
        },
        {
            'icon':'profile',
            'id': 1,
            'fname': 'Mirabel S',
            'lname': 'Gonzalez',
            'title':'Gonzalez, Mirabel S',
            'last_seen':' 4/10/2018',                
            'action':'profile-details',
            'image':1,
            'day':'Today',
            'assigned':1,
            'dob': '04/28-1984',
            'sex': 'Female',
            'phone': '444-555-4444',
            'profile_image': 'profile-pictures.jpg',
            'images': [
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
            ],
            'allergies': ['Eggs, latex'],
            'medications': [
                {'name':'XYZ', 'prescribed':'4/10/2018'},
                {'name':'XYZ', 'prescribed':'4/10/2018'}
            ],
            'diagnosis': ["HIV (B20)", "Opiate use disorder (F11.24)"]                
        },
        {
            'icon':'img-icon.png',
            'id': 2,
            'fname': 'Carmen A',
            'lname': 'Nardi',
            'title':' Nardi, Carmen A',
            'last_seen':' 1/12/2018',                
            'action':'profile-details',
            'image':1,
            'day':'Today',
            'assigned':0,
            'dob': '05/28-1984',
            'sex': 'Male',
            'phone': '444-555-5555',
            'profile_image': 'profile-pictures.jpg',
            'images': [
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
                'profile-pictures.jpg',
            ],
            'allergies': ['Eggs, latex'],
            'medications': [
                {'name':'XYZ', 'prescribed':'4/10/2018'},
                {'name':'XYZ', 'prescribed':'4/10/2018'}
            ],
            'diagnosis': ["HIV (B20)", "Opiate use disorder (F11.24)"]                                             
        },
    ],
    ol:[
        {
            'icon':'zmdi-accounts',
            'title':'Outreach List',
            'action':'ol'

        },
        {
            'icon':'zmdi-search',
            'title':'Person Search',
            'action':'search'
        },
        {
            'icon':'zmdi-assignment-o',
            'title':'New Encounter Form',
            'action':'nef'
        },
        {
            'icon':'zmdi-receipt',
            'title':'New Quick Note',
            'action':'nqn'                
        },
        {
            'icon':'zmdi-sign-in',
            'title':'Sign Out',
            'action':'login'
            
        }
    ]
};